...
| 채널 | 피드 | 제목 |
|---|---|---|
| Light | 국내 (Naver) | `📌 등대 포트폴리오 뉴스피드 (국내)` |
| Atom | 국내 (Naver) | `📌 김아톰 포트폴리오 뉴스피드` |
| Light | 해외 (ENG) | `📌 등대 포트폴리오 뉴스피드 (해외)` |

### 5.5 구글 시트 기록 (`sheets_writer.py`)

- 피드 실행 후 수집된 뉴스를 로그 시트 **2행에 삽입** (기존 데이터 아래로 밀림)
- Light 국내 → `News1` 시트, Atom 국내 → `News2` 시트, ENG 해외 → `News3` 시트
